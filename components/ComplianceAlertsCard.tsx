...
import type { ComplianceAlert } from '../lib/types/api'
...
export interface ComplianceAlertsCardRef {
  refresh: () => Promise<void>
}

export const ComplianceAlertsCard = forwardRef<ComplianceAlertsCardRef, ComplianceAlertsCardProps>(
  ({ businessId, title = 'Compliance Alerts', onAlertsSummaryChange }, ref) => {
  ...
  const loadAlerts = async (shouldCheck = false) => {
    ...
    try {
      ...
      if (shouldCheck) {
        checkResponse = await complianceApi.check(businessId, { forceRefresh: false })
        setCheckMessage(checkResponse.message)
      }

      // Fetch alerts
      const response = await complianceApi.getAlerts(businessId, {
      ...
      if (response.alerts.length > 0 && __DEV__) {
        console.log('ComplianceAlertsCard: Alert date fields - detectedAt:', response.alerts[0].detectedAt, 'createdAt:', response.alerts[0].createdAt)
      }
      
      setAlerts(response.alerts)
      setUnreadCount(response.unreadCount)
      // Update global unread count for tab badge
      setComplianceAlertsCount(response.unreadCount)
  ...
  const handleDismiss = async (alertId: string) => {
    try {
      await complianceApi.dismissAlert(alertId, businessId)
      ...
      // Update global unread count for tab badge
      setComplianceAlertsCount(newCount)
    } catch (err) {
  ...
  const formatDate = (dateValue: string | number | { seconds?: number; nanoseconds?: number; toDate?: () => Date } | undefined) => {
    if (!dateValue) {
      console.warn('ComplianceAlertsCard: formatDate received undefined/null value')
      return 'Recently'
    }
    ...
    if (typeof dateValue === 'number') {
      ...
      if (dateValue.toDate && typeof dateValue.toDate === 'function') {
        ...
        console.warn('ComplianceAlertsCard: Unknown date object format:', dateValue)
        return 'Recently'
      }
    } else {
      console.warn('ComplianceAlertsCard: Unknown date type:', typeof dateValue, dateValue)
      return 'Recently'
    }
    
    // Check if date is valid
    if (isNaN(date.getTime())) {
      console.error('ComplianceAlertsCard: Invalid date parsed. Value:', dateValue, 'Type:', typeof dateValue)
      return 'Recently'
    }
    ...
    try {
      ...
      console.error('ComplianceAlertsCard: Error formatting date:', error)
      return 'Recently'
    }
  }
      ...
      {error ? (
        ...
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptyText}>
            {statusMessage || 'No compliance issues detected. Your business transactions appear audit-ready with no patterns that would typically trigger government scrutiny.'}
          </Text>
        </View>
      ) : (
        <>
...
ComplianceAlertsCard.displayName = 'ComplianceAlertsCard'
