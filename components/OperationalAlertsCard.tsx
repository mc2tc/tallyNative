...
import type { OperationalAlert } from '../lib/types/api'
...
export interface OperationalAlertsCardRef {
  refresh: () => Promise<void>
}

export const OperationalAlertsCard = forwardRef<OperationalAlertsCardRef, OperationalAlertsCardProps>(
  ({ businessId, title = 'Operations Alerts', onAlertsSummaryChange }, ref) => {
  ...
  const loadAlerts = async (shouldCheck = false) => {
    ...
    try {
      ...
      if (shouldCheck) {
        checkResponse = await operationalApi.check(businessId, { forceRefresh: false })
        setCheckMessage(checkResponse.message)
      }

      // Fetch alerts
      const response = await operationalApi.getAlerts(businessId, {
      ...
      if (response.alerts.length > 0 && __DEV__) {
        console.log('OperationalAlertsCard: Alert date fields - detectedAt:', response.alerts[0].detectedAt, 'createdAt:', response.alerts[0].createdAt)
      }
      
      setAlerts(response.alerts)
      setUnreadCount(response.unreadCount)
      // Update global unread count for tab badge
      setOperationsAlertsCount(response.unreadCount)
  ...
  const handleDismiss = async (alertId: string) => {
    try {
      await operationalApi.dismissAlert(alertId, businessId)
      ...
      // Update global unread count for tab badge
      setOperationsAlertsCount(newCount)
    } catch (err) {
  ...
  const formatDate = (dateValue: string | number | { seconds?: number; nanoseconds?: number; toDate?: () => Date } | undefined) => {
    if (!dateValue) {
      console.warn('OperationalAlertsCard: formatDate received undefined/null value')
      return 'Recently'
    }
    ...
    if (typeof dateValue === 'number') {
      ...
      if (dateValue.toDate && typeof dateValue.toDate === 'function') {
        ...
        console.warn('OperationalAlertsCard: Unknown date object format:', dateValue)
        return 'Recently'
      }
    } else {
      console.warn('OperationalAlertsCard: Unknown date type:', typeof dateValue, dateValue)
      return 'Recently'
    }
    
    // Check if date is valid
    if (isNaN(date.getTime())) {
      console.error('OperationalAlertsCard: Invalid date parsed. Value:', dateValue, 'Type:', typeof dateValue)
      return 'Recently'
    }
    ...
    try {
      ...
      console.error('OperationalAlertsCard: Error formatting date:', error)
      return 'Recently'
    }
  }
      ...
      {error ? (
        ...
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptyText}>
            {statusMessage || 'No operational issues detected. Your business operations appear optimized with no cost savings or efficiency opportunities identified at this time.'}
          </Text>
        </View>
      ) : (
        <>
...
OperationalAlertsCard.displayName = 'OperationalAlertsCard'
